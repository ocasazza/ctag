...
"""
Replace command module for the ctag CLI tool.

This module defines the 'replace' command for replacing tags on Confluence pages.
"""
...
def replace(ctx, cql_expression, tag_pairs, interactive, abort_key, cql_exclude):
    ...
    if dry_run:
        ...
        list(
            tag_mapping.keys())} with {
                list(
                    tag_mapping.values())} on '{title}' (Space: {space})"
            )
        return
